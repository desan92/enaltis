<template>
  <!--div on es carregara el mapa.-->
  <div class="chartdiv" ref="chartdiv"></div>
</template>

<script>
//imports dels elements d'amcharts.
...
var ABSSeries = chart.series.push(new am4maps.MapPolygonSeries());
    ...
    ABSSeries.hide();
    ABSSeries.geodataSource.events.on("done", function() {
...
imageSeries.dataSource.url = "api/regionsABS.json";
...
var backZoom = chart.createChild(am4core.ZoomOutButton);
    ...
    backZoom.hide();
    backZoom.events.on("hit", function() {
        if(imageSeries)
        {
            imageSeries.show();
        }
